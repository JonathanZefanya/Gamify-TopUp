...
trait Upload
{
    ...
    public function fileUpload($file, $location, $fileName = null, $size = null, $encodedFormat = null, $encodedQuality = 90, $oldFileName = null, $oldDriver = 'local')
    {
        ...
        // Guard against missing PHP FTP extension: fallback to sftp or local to avoid FTP_BINARY error
        if ($activeDisk === 'ftp' && !extension_loaded('ftp')) {
            $activeDisk = !empty(config('filesystems.disks.sftp.host')) ? 'sftp' : 'local';
        }
        ...
        if (!is_string($file)) {
            ...
            if (str_starts_with($file->getMimeType(), 'image/')) {
                ...
                $name = $fileName ?? (Str::random(40) . '.' . ($file->extension() ?: pathinfo($file->getPathname(), PATHINFO_EXTENSION)));
                $path = rtrim($location, '/').'/'.$name;
                Storage::disk($activeDisk)->put($path, file_get_contents($file->getPathname()));
            }
        } else {
    ...
    {
        if ($driver === 'ftp' && !extension_loaded('ftp')) {
            $driver = !empty(config('filesystems.disks.sftp.host')) ? 'sftp' : 'local';
        }
        if (!empty($old)) {
